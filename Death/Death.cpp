    ...
    for (int k = i + 1; k < n - 2; k++) {
        ...
        for (int l = i + 2; l < n - 1; l++) {
            ...
            for (int g = i + 3; g < n; g++) {
                   ...
                   //cout << "gg" << g << endl;
                   if (l != g && k != l && i != k && i!=g && l!=i && g!=l && k!=g) {
                ...
                if (check(arrx, arry)) {
                    ...
                    cout << "Фигуры из точек:" << endl;
                    cout << "| " << "x" << " | " << "y" << " |" << endl;
                    for (int y = 0; y < 4; y++) {
                        cout << "| " << arrx[y] << " | " << arry[y] << " |" << endl;
...
double findcentrex(int mid, int min, int max, double* arrx, double* arry) {
...
double findcentrey(int mid, int min, int max, double* arrx, double* arry) {
...
int prinadleshnost(double** mas, double r, double R, int n, double* arrx, double* arry, double xcen, double ycen, double max, double min) {
    int count = 0;
    int j = 0;
    for (int i = 0; i < n; i++) {
        //y=-(x1y2-x2y1)-(y1-y2)x/(x2-x1)
        double a = ((mas[i][j]-xcen) * (mas[i][j]-xcen) + (mas[i][j + 1]-ycen) * (mas[i][j + 1]-ycen));
        // cout << a << "r" << r << endl;
        // cout << a << "R" << R << endl;
        // cout << "y" << (arry[0] - arry[1]) * mas[i][j] + (arrx[0] * arry[1] - arrx[1] * arry[0]) + (arrx[1] - arrx[0]) * mas[i][j + 1] << endl;
         if (arry[0] == arry[1]) {

             if (a > r * r && a < R * R && mas[i][j + 1] > arry[0]) {
                 count++;

             }
         }
         else if (arrx[0] = arrx[1]) {
             if (a > r * r && a < R * R && mas[i][j] > arrx[0]) {
                 count++;

             }

         }
         else if ((arrx[0] - arry[0]) * mas[i][j] != 0 && (arrx[1] - arrx[0]) * mas[i][j + 1] != 0) {
...
int prinadleshnost1(double** mas, double r, double R, int n, double* arrx, double* arry, double xcen, double ycen, double max, double min) {
    int count = 0;
    int j = 0;
    for (int i = 0; i < n; i++) {
        //y=-(x1y2-x2y1)-(y1-y2)x/(x2-x1)
...
int* prinadleshnosttocka(double** mas, double r, double R, int n, double* arrx, double* arry, int count, double xcen, double ycen, double max, double min) {
    int j = 0;
    int k = 0;
...
int* prinadleshnosttocka1(double** mas, double r, double R, int n, double* arrx, double* arry, int count, double xcen, double ycen, int max, int min) {
    int j = 0;
    int k = 0;
    int* mass = new int[count];
    for (int i = 0; i < n; i++) {
...
int main() {
    ...
    protokol.open("protokol.txt", ios::out);
    for (int i = 0; i < n - 3; i++) {
        ...
        if (peremoga(mas, n, m, i) == true) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                protokol << "| " << arrx[d] << " | " << arry[d] << " |" << endl;
            }

            int max = findmaxx(arrx);
            protokol << "max spot" << max << endl;
            int min = findminx(arrx);
            protokol << "min spot" << min << endl;
            int ny = ihatenmax(max, arrx);
            protokol << "id max" << ny << endl;
            int nyx = ihatenmin(min, arrx);
            protokol << "id min" << ny << endl;
            int mid = bruhtella(max, min, arrx);
            protokol << "id midlle1" << mid << endl;
            int nyxmid = bruhtellanum(ny, nyx, mid, arrx);
            protokol << "id midlle2" << nyxmid << endl;


            double r = findcentreandraduism(nyxmid, nyx, ny, arrx, arry);
            protokol << "радиус малой окружности" << r << endl;
            double R = findcentreandraduisM(nyxmid, nyx, ny, arrx, arry);
            protokol << "радиус большой окружности" << r << endl;

            double xcen = findcentrex(nyxmid, nyx, ny, arrx, arry);
            double ycen = findcentrey(nyxmid, nyx, ny, arrx, arry);

            cout << "xc" << xcen << endl;
            cout << "r" << r << endl;
            cout << "yc" << ycen << endl;
            int count = prinadleshnost(mas, r, R, n, arrx, arry,xcen,ycen, max, min);
            int count2 = prinadleshnost1(mas, r, R, n, arrx, arry, xcen, ycen, max, min);
            //cout << "c=" << count;
            if (count1 < count) {
                up = true;
                count1 = count;
            ...
            if (count != 0) {
                int* dots = prinadleshnosttocka(mas, r, R, n, arrx, arry, count, xcen, ycen, max, min);
                int* dotsdown = prinadleshnosttocka1(mas, r, R, n, arrx, arry, count2, xcen, ycen, max, min);
            ...
            if (count2 != 0) {
                int* dotsdown = prinadleshnosttocka1(mas, r, R, n, arrx, arry, count1, xcen, ycen, max, min);
    ...
    */
    ...
    if (up) {
        ...
        file << "Точки:" << endl;
        int* dots = prinadleshnosttocka(mas, r1, R1, n, arrx1, arry1, count1, xcen1, ycen1, max1, min1);
        ...
        file << "Точки:" << endl;
        int* dots = prinadleshnosttocka1(mas, r1, R1, n, arrx1, arry1, count1, xcen1, ycen1, max1, min1);
        for (int i = 0; i < count1; i++) {
            int k = dots[i];
            //cout << dots[i] << endl;
            cout << "| " << mas[k][0] << " | " << mas[k][1] << " |" << endl;
